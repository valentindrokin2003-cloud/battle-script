...
Выполнено. Каждый файл прошёл настоящий red→green: тест писался и запускался первым (компиляция падала на неопределённых типах), затем добавлялась реализация. `make check` чист.
